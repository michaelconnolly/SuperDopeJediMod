...
//    }
    ...
    public ProbeDroidModel(float scale, float p_i1164_2_, int width, int height) {
            ...
            subpart.setTexSize(TEXTURE_WIDTH, TEXTURE_HEIGHT);
...
//        if (this.head != null) {
//            this.head.yRot = netHeadYaw * 0.017453292F; // degrees to radians
//        }
//
//        int i = 0;
//        for (ModelRenderer subrenderer : arms2) {
//
//            subrenderer.yRot = 0.0F;
//            subrenderer.xRot = 0.0F;
//            subrenderer.zRot = 0.0F;
//
//            // TODO: make this more random.
//            if (i == 0) {
//                subrenderer.xRot = .75F + MathHelper.cos(limbSwing /* * 0.6662F */) * 1.4F * limbSwingAmount * 1.0F;
//            } else if (i < 3) {
//                subrenderer.zRot = MathHelper.cos(limbSwing /* * 0.6662F */) * 1.4F * limbSwingAmount * 1.0F;
//            } else if (i < 4) {
//                subrenderer.xRot = .75F + MathHelper.cos(limbSwing/* * 0.6662F*/ + (float)Math.PI) * 1.4F * limbSwingAmount * 1.0F;
//            } else {
//                subrenderer.zRot     = MathHelper.cos(limbSwing /** 0.6662F*/ + (float)Math.PI) * 1.4F * limbSwingAmount * 1.0F;
//            }
//
//            i++;
//        }
//    }
    ...
    private void modelArms(ModelRenderer parent, float xOffset, float zOffset, boolean longStyle, ProbeDroidModel.ArmDirection armDirection, float angle, int i) {
        ...
        upperArm.setTexSize(TEXTURE_WIDTH, TEXTURE_HEIGHT);
        if (longStyle) {
            upperArmLength = 6;
            upperArm.texOffs(4, 53);
        } else {
            upperArmLength = 4;
            upperArm.texOffs(0, 53);
        }
        ...
        lowerArm.setTexSize(TEXTURE_WIDTH, TEXTURE_HEIGHT);
        lowerArm.texOffs(0, 53);
        if (longStyle ) {
            lowerArm.texOffs(4, 53);
        } else {
            lowerArm.texOffs(0, 53);
        }
        ...
        switch (armDirection) {
            case BACKWARD:
                upperArm.xRot = angle;
                yCoord = (float) (((float) upperArmLength) * Math.cos(upperArm.xRot));
                zCoord = (float) (((float) upperArmLength) * Math.sin(upperArm.xRot));
                break;
            ...
            case RIGHTWARD:
                upperArm.zRot = -angle;
                yCoord = (float) (((float) upperArmLength) * Math.cos(upperArm.zRot));
                xCoord = (float) (((float) -upperArmLength) * Math.sin(upperArm.zRot));
                break;
        ...
        this.renderers.add(upperArm);
        this.renderers.add(lowerArm);
    }
